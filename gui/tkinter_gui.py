...
class FinancialApp(tk.Tk):
    def __init__(self):
        ...
        self.charts(self.tab_infographics)
    ...
    def charts(self, parent: tk.Frame) -> None:
    ...
    def operations_tab(self, parent: tk.Frame) -> None:
        parent.grid_columnconfigure(1, weight=1)
        ...
        save_btn = tk.Button(form_frame, text="Save", command=save_record_inline)
        ...
        def import_records():
            ...
            filepath = filedialog.askopenfilename(
                ...
                title=f"Select {desc} file to import",
            )
            if not filepath:
                return
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success",
                    f"Successfully imported {imported_count} records from {desc} file.\nAll existing records have been replaced.",
                )
                self._refresh_list()
                self._refresh_charts()

            except FileNotFoundError:
                logger.exception(f"{desc} import file not found: %s", filepath)
                messagebox.showerror("Error", f"File not found: {filepath}")
            except Exception as e:
                logger.exception(f"Failed to import {desc} from %s", filepath)
                messagebox.showerror("Error", f"Failed to import {desc}: {str(e)}")
    ...
    def reports_tab(self, parent: tk.Frame) -> None:
        parent.grid_rowconfigure(1, weight=1)
        parent.grid_columnconfigure(0, weight=1)
    ...
    def settings_tab(self, parent: tk.Frame) -> None:
        parent.grid_columnconfigure(1, weight=1)
        ...
        current_panel = {"add": None, "report": None}
        ...
        def import_mand():
            ...
            filepath = filedialog.askopenfilename(
                defaultextension=ext,
                filetypes=[(f"{fmt} files", f"*{ext}"), ("All files", "*.*")],
                title=f"Select {desc} file to import mandatory expenses",
            )
            if not filepath:
                return
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success",
                    f"Successfully imported {len(expenses)} mandatory expenses from {desc} file.\nAll existing mandatory expenses have been replaced.",
                )
                refresh_mandatory()
                self._refresh_charts()
