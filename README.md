...
- **Import from Excel** — Импорт данных из XLSX файла
...
Дополнительно: в окне генерации отчёта доступна кнопка **Export to XLSX** для сохранения отчёта в формате Excel (.xlsx). Файл содержит лист `Report` с колонками `Date, Type, Category, Amount (KZT)`. Если указан начальный остаток, он записывается отдельной строкой с `Type = Initial Balance`.
...
3. Используйте кнопки:
   ...
   - **Import from CSV** — Импорт обязательных расходов из CSV файла
   - **Export to CSV** — Экспорт обязательных расходов в CSV файл
   - **Import from Excel** — Импорт обязательных расходов из Excel (.xlsx)
   - **Export to Excel** — Экспорт обязательных расходов в Excel (.xlsx)
   - **Close** — Закрыть окно
...
- Вторая строка может содержать начальный остаток `Initial Balance` с пустыми полями `Type` и `Category`
...
1. Нажмите "Import from Excel" в главном окне.
2. Выберите файл `.xlsx` в диалоговом окне открытия файла.
3. Подтвердите импорт (все существующие записи будут заменены).
4. Приложение покажет количество импортированных записей.
...
4. Приложение экспортирует все обязательные расходы в CSV формат.
...
1. Нажмите "Manage Mandatory" для открытия окна управления обязательными расходами.
2. Нажмите "Import from CSV" или "Import from Excel".
3. Выберите CSV/XLSX-файл с обязательными расходами в диалоговом окне.
