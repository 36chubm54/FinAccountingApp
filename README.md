...
После запуска `python main.py` откроется окно с девятью кнопками и блоком инфографики:
...
- Селектор формата для импорта/экспорта (CSV, XLSX)
- **Import** — Импорт данных из файла выбранного формата

Справа отображается инфографика:
- Круговая диаграмма расходов по категориям
- Гистограмма доходов/расходов по дням месяца (с переключением месяца)
- Гистограмма доходов/расходов по месяцам года (с переключением года)

Доходы показаны зелёным цветом в положительной плоскости, расходы — красным в отрицательной.
При большом количестве категорий мелкие значения объединяются в сегмент «Прочее».
...
Результат отобразится в текстовом поле. Для таблиц используется форматированная таблица с итогом. После основного вывода добавляется таблица доходов и расходов по прошедшим и текущему месяцам (по выбранному периоду, либо по последнему году с данными).

Дополнительно: в окне генерации отчёта доступна кнопка **Export** для сохранения отчёта в любом доступном формате. Excel-файл содержит лист `Report` с колонками `Date, Type, Category, Amount (KZT)` и отдельный лист `Yearly Report` с помесячной сводкой доходов и расходов. В PDF-файле эта сводка добавляется второй таблицей сразу после выписки. Если указан начальный остаток, он записывается отдельной строкой с `Type = Initial Balance`.
...
3. Используйте кнопки:
   ...
   - **Import** — Импорт обязательных расходов из файла выбранного формата (CSV, XLSX)
...
При экспорте также добавляется лист `Yearly Report` с помесячной сводкой доходов и расходов (для импорта читается только первый лист книги, остальные листы игнорируются).
...
> **Примечание:** Формат CSV необходим для бэкапа данных. Формат PDF для обязательных расходов имеет табличную структуру с соответствующими колонками и заголовками. Он поддерживается только для экспорта, импорт PDF не поддерживается.

#### Импорт обязательных расходов

1. Нажмите "Manage Mandatory" для открытия окна управления обязательными расходами.
2. Выберите нужный вам формат (CSV или XLSX) и нажмите "Import". Импорт PDF не поддерживается.
...
#### Импорт CSV для обязательных расходов

1. Выберите формат `CSV` в выпадающем списке формата импорта в окне управления обязательными расходами.
2. Нажмите "Import".
3. Выберите файл `.csv` и подтвердите импорт (все существующие обязательные расходы будут заменены).
4. Файл должен содержать колонки `Amount (KZT),Category,Description,Period`.
5. Поддерживаемые периоды: `daily`, `weekly`, `monthly`, `yearly`.

#### Импорт XLSX для обязательных расходов

1. Выберите формат `XLSX` в выпадающем списке формата импорта в окне управления обязательными расходами.
2. Нажмите "Import".
3. Выберите файл `.xlsx` и подтвердите импорт (все существующие обязательные расходы будут заменены).
4. Файл должен содержать лист `Mandatory` и колонки `Amount (KZT),Category,Description,Period`.
5. Поддерживаемые периоды: `daily`, `weekly`, `monthly`, `yearly`.
...
- Приложение теперь поддерживает экспорт отчетов и обязательных расходов в формате `PDF`. PDF-файлы, создаваемые приложением, содержат таблицы, с соответствующими колонками и заголовками.
